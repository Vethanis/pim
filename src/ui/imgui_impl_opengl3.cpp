...
// Functions
extern "C" int32_t ImGui_ImplOpenGL3_Init()
{
    GLint major, minor;
...
extern "C" void ImGui_ImplOpenGL3_Shutdown()
{
    ImGui_ImplOpenGL3_DestroyDeviceObjects();
}

extern "C" void ImGui_ImplOpenGL3_NewFrame()
{
    if (!g_ShaderHandle)
        ImGui_ImplOpenGL3_CreateDeviceObjects();
}

static void ImGui_ImplOpenGL3_SetupRenderState(ImDrawData* draw_data, int32_t fb_width, int32_t fb_height, GLuint vertex_array_object)
{
            ...
            else
            {
                ...
                if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                {
                    ...
                    if (clip_origin_lower_left)
                        glScissor((int32_t)clip_rect.x, (int32_t)(fb_height - clip_rect.w), (int32_t)(clip_rect.z - clip_rect.x), (int32_t)(clip_rect.w - clip_rect.y));
                    else
                        glScissor((int32_t)clip_rect.x, (int32_t)clip_rect.y, (int32_t)clip_rect.z, (int32_t)clip_rect.w); // Support for GL 4.5 rarely used glClipControl(GL_UPPER_LEFT)
...
extern "C" int32_t ImGui_ImplOpenGL3_CreateFontsTexture()
{
    // Build texture atlas
    ImGuiIO& io = ImGui::GetIO();
    uint8_t* pixels;
...
static int32_t CheckShader(GLuint handle, const char* desc)
{
    ...
    {
        ImVector<char> buf;
        buf.resize((int32_t)(log_length + 1));
...
static int32_t CheckProgram(GLuint handle, const char* desc)
{
    GLint status = 0, log_length = 0;
...
extern "C" int32_t ImGui_ImplOpenGL3_CreateDeviceObjects()
{
    // Backup GL state
