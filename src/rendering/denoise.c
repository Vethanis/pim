...
static bool LogErrors(void)
{
    bool hadError = false;
    const char* msg = NULL;
    while (oidnGetDeviceError(ms_device, &msg) != OIDN_ERROR_NONE)
    {
        hadError = true;
...
static bool EnsureInit(void)
{
    if (!ms_once)
    {
        ...
        ms_device = oidnNewDevice(OIDN_DEVICE_TYPE_DEFAULT);
        ASSERT(ms_device);
        if (!ms_device)
        {
            LogErrors();
...
static OIDNFilter NewFilter(const CacheKey* key)
{
    ...
    OIDNFilter filter = oidnNewFilter(ms_device, typeStr);
    ASSERT(filter);
    if (!filter)
    {
        LogErrors();
...
static OIDNFilter GetFilter(const CacheKey* key)
{
    ...
    if (i == -1)
    {
        ...
        if (ms_cacheValues[i])
        {
            oidnReleaseFilter(ms_cacheValues[i]);
