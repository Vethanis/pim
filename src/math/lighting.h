...
// NOTE: output should be divided by PI
...
// Specular 'G' term
// represents the self shadowing of microfacets in rough materials
// https://seblagarde.files.wordpress.com/2015/07/course_notes_moving_frostbite_to_pbr_v32.pdf
// Page 12, Listing 2
pim_inline float VEC_CALL G_SmithGGX(float NoL, float NoV, float alpha)
{
    float a2 = alpha * alpha;
    // NoL and NoV purposely reversed
    float v = NoL * sqrtf((-NoV * a2 + NoV) * NoV + a2);
    float l = NoV * sqrtf((-NoL * a2 + NoL) * NoL + a2);
    return 0.5f / (v + l);
}

// multiply output by radiance of light
// dot(N, L) is already applied
    ...
    float metallic)
{
    ...
    float4 specular = f4_divvs(DFG, f1_max(1e-5f, 4.0f * NoL * NoV));

    float4 kD = f4_mulvs(f4_inv(F), 1.0f - metallic);
    float4 diffuse = f4_divvs(f4_mul(albedo, kD), kPi);

    return f4_mulvs(f4_add(diffuse, specular), NoL);
}
    ...
    float ao)           // 1 - ambient occlusion (affects gi only)
{
    ...
    float4 DFG = EnvDFG(f0, cosTheta, alpha);
    float4 specular = f4_divvs(DFG, f1_max(1e-5f, 4.0f * cosTheta * cosTheta));

    float4 kD = f4_mulvs(f4_inv(F), 1.0f - metallic);
    float4 diffuse = f4_mul(albedo, kD);

    specular = f4_mul(specular, specularGI);
    diffuse = f4_mul(diffuse, diffuseGI);
    return f4_mulvs(f4_add(diffuse, specular), ao);
}
