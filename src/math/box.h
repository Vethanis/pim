...
pim_inline float4 VEC_CALL box_size(Box3D box)
{
    return f4_sub(box.hi, box.lo);
}

pim_inline float4 VEC_CALL box_extents(Box3D box)
{
    return f4_mulvs(box_size(box), 0.5f);
}

pim_inline bool VEC_CALL box_contains(Box3D box, float4 pt)
{
...
pim_inline Box3D VEC_CALL box_union(Box3D lhs, Box3D rhs)
{
    return box_new(f4_min(lhs.lo, rhs.lo), f4_max(lhs.hi, rhs.hi));
}

pim_inline Box3D VEC_CALL box_intersect(Box3D lhs, Box3D rhs)
{
...
pim_inline Box3D VEC_CALL box_transform(float4x4 matrix, Box3D box)
{
